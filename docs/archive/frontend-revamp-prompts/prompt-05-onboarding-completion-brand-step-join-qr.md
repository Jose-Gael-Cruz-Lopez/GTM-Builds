# Prompt 5 — Onboarding Completion (Brand step, Join QR)

> **How to run this prompt.** Paste **Prompt 0 (Master System Prompt)** below into your coding agent first, then paste the **Prompt 5** body. The Master prompt encodes the design language, tokens, fonts, backend contracts, and conventions every surface must follow.
...
**Closes:** P0.3 onboarding completion (brand step + printable join QR + share link).

```
Complete the existing `/onboarding` flow. Two of three steps work; finish step 1 (Brand) and step 3 (Finish) to feel like a Cleo/Shopify-grade product setup.

REFERENCES TO FUSE:
  • Renaissance Edition — premium feature-card grid for the finish step.
  • Cleo AI — warm cream card preview that updates live as the owner picks a color.
  • MindMarket — illustrated step indicators at the top.

FILE: `frontend/src/routes/onboarding.tsx` (modify) + new components in `frontend/src/components/onboarding/`.

STEP 1 — BRAND (currently placeholder):
  • Layout: left form, right live preview of the customer's wallet card.
  • Fields:
    - Logo (Supabase Storage upload → bucket `business-logos`, max 1MB, square crop with `react-easy-crop`). Saves `logo_url` to business row via `PATCH /businesses/:id`.
    - Primary color picker (preset swatches: ink, signal yellow, celebrate red, health chartreuse, scanner rose, plus a custom hex input). Saves `primary_color`.
    - Tagline (optional, max 60 chars).
  • Live preview component re-renders the `LoyaltyCardPreview` with the chosen logo + color.

STEP 3 — FINISH:
  • Three editorial feature cards (Shopify grid):
    1. "Comparte con tus clientes" — show the generated join URL (`https://app.../join/${businessId}`) with a copy button + WhatsApp share button (pre-filled message: "¡Hola! Únete a mi programa de lealtad y gana recompensas: [url]").
    2. "Imprime tu QR" — show the join QR (rendered with `qrcode.react`, 320px, ink foreground on cream). Download buttons: PNG (use `qrcode.react`'s ref + canvas), PDF (use `jspdf` — install it — at A6 size with business name and tagline). 
    3. "Configura tu primer staff key" — show the staff key created in step 2 with a copy button and a warning ("Guárdalo: solo se muestra una vez").
  • Final CTA: signal-yellow "Ir a mi panel" → `/dashboard/${businessId}`.

STEP INDICATOR:
  • Replace the existing indicator with three illustrated dots that fill on completion. Active dot pulses subtly.

ACCEPTANCE:
  • Owner finishes onboarding and has: logo uploaded, color saved, reward configured, staff key created, join URL ready to share, QR downloadable.
  • Refreshing mid-onboarding restores the user to the correct step using a `?step=` query param.
```
