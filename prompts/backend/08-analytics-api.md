# Agent Prompt — 08: Analytics API

## Context

You are building the **NexoLeal Cloudflare Workers backend** — Wave 3B. Your job is to implement the analytics endpoints that power the admin dashboard. This runs in parallel with Visits and Campaigns agents.
...
- `src/types/db.ts` — all row types
- `src/types/env.ts` — Env with `ANALYTICS_CACHE` KV binding

Analytics responses are expensive (aggregate queries across many rows). Always check the `ANALYTICS_CACHE` KV namespace first and return cached data if fresh. Cache TTL is 5 minutes for most endpoints, 1 hour for peak-hours heatmap.

All response shapes must be ready for direct consumption by Lovable (React + recharts/chart.js). Use `{ labels: string[], values: number[] }` for time-series data.
...
|--------|------|-----------|-------------|
| GET | `/businesses/:id/analytics/retention` | 5 min | Retention rate for 30/60/90d windows |
| GET | `/businesses/:id/analytics/visits` | 5 min | Daily visit volume time-series |
| GET | `/businesses/:id/analytics/clients` | 5 min | New vs returning clients breakdown |
| GET | `/businesses/:id/analytics/peak-hours` | 60 min | Busiest hours/days heatmap |
| GET | `/businesses/:id/analytics/churn-risk` | 5 min | Clients approaching lost status |
...
// ...
// Analytics routes are nested under /businesses/:id/analytics
app.route('/businesses', analyticsRoutes)

// This means the full paths become:
// GET /businesses/:id/retention
// GET /businesses/:id/visits
// etc.
```

Wait — since analyticsRoutes uses `/:id/retention` etc., mounting at `/businesses` makes the final path `/businesses/:id/retention`. Verify this is correct and matches what the frontend expects.
...
Zero TypeScript errors. Ensure all `grid[day]?.[hour]` accesses use optional chaining since `noUncheckedIndexedAccess` is enabled in tsconfig.
