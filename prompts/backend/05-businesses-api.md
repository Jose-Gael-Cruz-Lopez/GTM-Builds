# Agent Prompt — 05: Businesses API

## Context

You are building the **NexoLeal Cloudflare Workers backend** — Wave 2C. Your job is to implement all business management endpoints. This runs in parallel with Auth, Token Engine, and Clients agents.

Wave 1 has already created the project scaffold, Supabase client, and all types. Read:
- `src/types/env.ts` — Env bindings
- `src/types/api.ts` — `ok()`, `err()`, `parsePagination()`, `ApiResponse`
- `src/types/db.ts` — `BusinessRow`, `BusinessInsert`, `LoyaltyConfigRow`, `StaffKeyRow`
- `src/lib/supabase.ts` — `createSupabaseClient()`, `mapSupabaseError()`

When this agent finishes, the Auth agent's `requireAdmin()` and `requireClient()` middleware will already exist (also Wave 2). Import them and use them on routes.
...
|--------|------|------|-------------|
| POST | `/businesses` | Admin JWT (owner) | Onboard new business |
| GET | `/businesses/:id` | Any auth | Get business profile |
| PATCH | `/businesses/:id` | Admin | Update business settings |
| GET | `/businesses/:id/loyalty-config` | Any auth | Get loyalty rules |
| PATCH | `/businesses/:id/loyalty-config` | Admin | Update loyalty rules |
| POST | `/businesses/:id/staff-keys` | Admin | Generate new staff API key |
| GET | `/businesses/:id/staff-keys` | Admin | List staff keys |
| DELETE | `/businesses/:id/staff-keys/:keyId` | Admin | Deactivate staff key |
| GET | `/businesses/:id/stats/summary` | Admin | Dashboard summary stats |

---

## Step 1 — `src/routes/businesses.ts`

Create `/backend/src/routes/businesses.ts`:
...
Uncomment the business route in `src/index.ts`:
