      ...
      <AppShell variant="dark">
        <div className="mx-auto max-w-md px-4 py-12">
          ...
          <p className="mt-2 text-sm text-[color:var(--color-cream)]/70">
            Cuéntanos cómo te llamas para crear tu perfil.
          </p>
          <form
            onSubmit={(e) => {
              e.preventDefault();
              if (fullName.trim()) bootstrap.mutate(fullName.trim());
            }}
    ...
    <AppShell variant="dark">
      <div className="mx-auto max-w-md px-4 py-8 pb-24">
        ...
        {list.length === 0 ? (
          <div className="mt-8">
            ...
            >
              <Label htmlFor="code" className="text-[color:var(--color-cream)]/80">
                ¿Tienes un código?
              </Label>
              <Input
                id="code"
      ...
      <div className="fixed inset-x-0 bottom-0 z-30 border-t border-white/5 bg-[rgba(13,13,13,0.85)] px-4 py-3 backdrop-blur-md">
        <div className="mx-auto flex max-w-md items-center justify-between">
          <Link
            to="/wallet/profile"
            className="inline-flex items-center gap-2 text-sm text-[color:var(--color-cream)]/80"
          >
      ...
      <Dialog open={nameOpen} onOpenChange={setNameOpen}>
        <DialogContent>
          ...
          <DialogFooter>
            <Button
              onClick={() => bootstrap.mutate(fullName)}
              disabled={bootstrap.isPending || !fullName.trim()}
            >
