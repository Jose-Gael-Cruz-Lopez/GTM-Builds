...
function ForgotPasswordPage() {
  ...
  return (
    <AuthSplit headline="¿Olvidaste tu contraseña?" subtitle="Te enviamos un enlace para crear una nueva en segundos.">
      ...
      {!sent ? (
        <form onSubmit={onSubmit} className="mt-6 space-y-4">
          ...
          <p className="text-center text-xs">
            <Link to="/login" className="underline text-[color:var(--color-ink-soft)]">Volver a iniciar sesión</Link>
          </p>
        </form>
      ) : (
        <div className="surface-paper mt-6 p-6 text-center">
          <h3 className="font-display text-xl">Te enviamos un enlace.</h3>
          <p className="mt-2 text-sm text-[color:var(--color-ink-soft)]">Revisa <strong>{email}</strong>. Si no llega, intenta de nuevo en {cooldown}s.</p>
