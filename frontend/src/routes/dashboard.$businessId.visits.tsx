...
function VisitsPage() {
  ...
  const rows = (list.data?.visits ?? []).filter((v) =>
    rewardsOnly ? v.reward_unlocked : true,
  )
      ...
      <div className="mb-6 flex flex-wrap items-end justify-between gap-4">
        ...
        <div className="flex items-center gap-2">
          <Switch
            id="rewards-only"
            checked={rewardsOnly}
            onCheckedChange={setRewardsOnly}
          />
    ...
    <TableRow>
      ...
      <TableCell>
        {visit.reward_unlocked ? (
          <Badge variant="outline" className="border-[color:var(--color-celebrate)] text-[color:var(--color-celebrate)]">
            Desbloqueada
          </Badge>
        ) : (
