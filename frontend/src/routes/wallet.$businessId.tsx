...
function WalletDetail() {
  ...
  useEffect(() => {
    ...
    const channel = supabase
      ...
      .on('postgres_changes', { event: 'INSERT', schema: 'public', table: 'visits', filter: `business_id=eq.${businessId}` }, (payload) => {
    ...
    <AppShell variant="dark" showNav={false}>
      ...
      <header className="sticky top-0 z-30 border-b border-white/5 bg-[rgba(13,13,13,0.85)] backdrop-blur-md">
        <div className="mx-auto flex max-w-md items-center justify-between px-4 py-3">
          ...
          <button
            type="button"
            onClick={() => qc.invalidateQueries({ queryKey: ['client', 'me', 'loyalty', businessId] })}
      ...
      <main className="mx-auto max-w-md px-4 py-6 pb-24">
        ...
        <article className="surface-card overflow-hidden p-6 text-[color:var(--color-ink)]">
          <div className="flex items-start justify-between">
            <div>
              <p className="text-xs text-[color:var(--color-ink-soft)] capitalize">{business.category}</p>
          ...
          <div className="mt-6 border-t border-[color:var(--color-border)] pt-4">
            <div className="flex items-baseline justify-between">
              <p className="text-xs text-[color:var(--color-ink-soft)]">Te faltan {l.stampsRemaining} sellos</p>
          ...
          {history.isLoading ? (
            ...
            <ul className="mt-6 space-y-2">
              {history.data.visits.map((v) => (
                <li key={v.id} className="flex items-center justify-between rounded-xl border border-white/5 bg-[var(--color-bg-elevated)] px-4 py-3">
                  <div>
                    <p className="text-sm text-[color:var(--color-cream)]">{formatRelative(v.created_at)}</p>
                    <p className="text-xs text-[color:var(--color-cream)]/50">{new Date(v.created_at).toLocaleDateString('es-MX', { day: 'numeric', month: 'short', hour: '2-digit', minute: '2-digit' })}</p>
                  </div>
            ...
            <p className="mt-6 text-sm text-[color:var(--color-cream)]/60">Aún no tienes visitas registradas. ¡Pasa por el negocio y muestra tu QR!</p>
          )}
        </section>
...
function QRBottomSheet({
  ...
  if (!open) return null
  return (
    <div className="fixed inset-0 z-50 flex flex-col items-center justify-end bg-black/60 backdrop-blur-sm" onClick={onClose}>
      <div
      ...
      >
        ...
        <div className="mt-8 flex flex-col items-center">
          ...
          <p className="mt-6 text-center text-sm text-[color:var(--color-cream)]/80">
            ...
            <span className="font-semibold">{secondsLeft}s</span> restantes — se renueva automáticamente.
          </p>
        </div>
      </div>
    </div>
...
function DemoWallet({
  ...
  return (
    <AppShell variant="dark" showNav={false}>
      <header className="sticky top-0 z-30 border-b border-white/5 bg-[rgba(13,13,13,0.85)] backdrop-blur-md">
        <div className="mx-auto flex max-w-md items-center justify-between px-4 py-3">
          <button onClick={onBack} className="inline-flex items-center gap-1 text-sm text-[color:var(--color-cream)]/70">
            <ArrowLeft className="h-4 w-4" /> Volver
          </button>
          <span className="text-xs uppercase tracking-wider text-[color:var(--color-cream)]/40">Demo</span>
        </div>
      </header>
      ...
      <main className="mx-auto max-w-md px-4 py-6 pb-24">
        ...
        <Button onClick={onOpenQr} className="btn-signal mt-6 inline-flex w-full items-center justify-center gap-2">
        ...
        <div className="mt-10 rounded-2xl border border-[color:var(--color-signal)]/30 bg-[var(--color-bg-elevated)] p-5">
          <div className="flex items-start gap-3">
            ...
            <div>
              <p className="font-display text-base text-[color:var(--color-cream)]">Esto es una demo</p>
