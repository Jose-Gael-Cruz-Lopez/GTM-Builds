 ...
 */
export function AppShell({ children, variant = 'light', showNav = true, className }: AppShellProps) {
  useEffect(() => {
  ...
  return (
    <div className={cn('min-h-screen', variant === 'dark' ? 'bg-[var(--color-bg-base)] text-[var(--color-cream)]' : 'bg-[var(--color-bg-paper)] text-[var(--color-ink)]', className)}>
      {showNav && (
        <header className={cn('sticky top-0 z-40 backdrop-blur-md', variant === 'dark' ? 'bg-[rgba(13,13,13,0.7)] border-b border-white/5' : 'bg-[rgba(249,246,239,0.8)] border-b border-[color:var(--color-border)]')}>
          <nav className="mx-auto flex max-w-7xl items-center justify-between px-4 py-3 md:px-8">
            ...
            <div className="flex items-center gap-3">
              {isAuthed ? (
                <>
                  {businessId ? (
                    <Link to="/dashboard/$businessId" params={{ businessId }} className="text-sm font-medium hover:opacity-80">
                      Mi panel
                    </Link>
                  ) : (
                ...
                <>
                  <Link to="/login" className="text-sm font-medium hover:opacity-80">Iniciar sesión</Link>
                  <Link
                    to="/signup"
...
export const AppShellLight = (p: Omit<AppShellProps, 'variant'>) => <AppShell {...p} variant="light" />
export const AppShellDark = (p: Omit<AppShellProps, 'variant'>) => <AppShell {...p} variant="dark" />
